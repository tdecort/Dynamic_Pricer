...
# =============================================================================
# 1. PARAMÈTRES GLOBAUX FIXES 
# =============================================================================
N = 1000              
# Note : marge_cible et budget_max sont maintenant calculés dynamiquement dans le callback

# Fonctions utilitaires
def quant_reverse_engineer(points_payoff):
    S_pts, Y_pts = [p[0] for p in points_payoff], [p[1] for p in points_payoff]
    cash = Y_pts[0]
    stocks = (Y_pts[1] - Y_pts[0]) / (S_pts[1] - S_pts[0]) if len(S_pts) > 1 else 0
    options = []
    
    for i in range(1, len(points_payoff) - 1):
        K = S_pts[i]
        p_avant = (Y_pts[i] - Y_pts[i-1]) / (S_pts[i] - S_pts[i-1])
        p_apres = (Y_pts[i+1] - Y_pts[i]) / (S_pts[i+1] - S_pts[i])
        chgt = p_apres - p_avant
        if abs(chgt) > 1e-5: options.append({'Strike': K, 'Quantite': chgt})

    portfolio = {'cash': cash, 'stocks': stocks, 'puts': [], 'calls': []}
    for opt in options:
        K, Q = opt['Strike'], opt['Quantite']
        if portfolio['stocks'] > 0 and Q < 0 and abs(portfolio['stocks'] + Q) < 1e-5:
            portfolio['cash'] += portfolio['stocks'] * K
            portfolio['puts'].append({'Strike': K, 'Quantite': Q})
            portfolio['stocks'] = 0 
        elif portfolio['stocks'] < 0 and Q > 0 and abs(portfolio['stocks'] + Q) < 1e-5:
            portfolio['cash'] -= abs(portfolio['stocks']) * K
            portfolio['puts'].append({'Strike': K, 'Quantite': Q})
            portfolio['stocks'] = 0
        elif Q > 0: portfolio['calls'].append({'Strike': K, 'Quantite': Q})
        elif Q < 0: portfolio['calls'].append({'Strike': K, 'Quantite': Q})
    return portfolio
...
def eval_portfolio_MC(portfolio, ST_array):
    ...
    for call in portfolio['calls']: val += call['Quantite'] * np.maximum(ST_array - call['Strike'], 0)
    for put in portfolio['puts']: val += put['Quantite'] * np.maximum(put['Strike'] - ST_array, 0)
    return val
...
def eval_portfolio_3D(portfolio, S_m, T_m, r_val, q_val, sigma_val):
    ...
    for call in portfolio['calls']: MtM += call['Quantite'] * bs_call(S_m, call['Strike'], T_m, r_val, q_val, sigma_val)
    for put in portfolio['puts']: MtM += put['Quantite'] * bs_put(S_m, put['Strike'], T_m, r_val, q_val, sigma_val)
    return MtM
...
def create_html_table(rows, color):
    header = html.Tr([
        ...
        html.Th("QUANTITÉ", style={'padding': '12px', 'textAlign': 'right', 'color': T_COLORS['text_main']}),
        html.Th("STRIKE", style={'padding': '12px', 'textAlign': 'right', 'color': T_COLORS['text_main']}),
        html.Th("PRIX UNITAIRE", style={'padding': '12px', 'textAlign': 'right', 'color': T_COLORS['text_main']}),
        html.Th("COÛT TOTAL", style={'padding': '12px', 'textAlign': 'right', 'color': color})
    ], style={'borderBottom': f'2px solid {color}'})

    body_rows = []
    for row in rows:
        action_color = T_COLORS['accent_green'] if row['Action'] in ['VENTE', 'PROFIT'] else T_COLORS['accent_red'] if row['Action'] == 'ACHAT' else T_COLORS['text_kpi']
        body_rows.append(html.Tr([
            ...
            html.Td(row['Quantite'], style={'padding': '10px', 'textAlign': 'right', 'color': T_COLORS['text_kpi']}),
            html.Td(row['Strike'], style={'padding': '10px', 'textAlign': 'right', 'color': T_COLORS['text_kpi']}),
            html.Td(row['Prix_U'], style={'padding': '10px', 'textAlign': 'right', 'color': T_COLORS['text_kpi']}),
...
# =============================================================================
# 5. GÉNÉRATION DE L'INTERFACE UTILISATEUR (DASH)
# =============================================================================
...
app.layout = html.Div(style={'backgroundColor': T_COLORS['bg_main'], 'fontFamily': T_COLORS['font'], 'padding': '40px', 'minHeight': '100vh'}, children=[
    ...
    html.H1("QUANT DESK // DYNAMIC PRICER", style={'color': T_COLORS['text_kpi'], 'textAlign': 'center', 'fontWeight': '300', 'letterSpacing': '4px', 'marginBottom': '40px'}),
    ...
    html.Div(style={**GLASS_STYLE, 'marginBottom': '40px', 'padding': '30px', 'display': 'flex', 'gap': '30px', 'alignItems': 'flex-end'}, children=[
        html.Div([
            html.Label("TICKER SOUS-JACENT", style={'color': T_COLORS['text_main'], 'fontSize': '12px', 'letterSpacing': '1px'}),
        ...
        html.Div([
            html.Label("MATURITÉ CIBLE", style={'color': T_COLORS['text_main'], 'fontSize': '12px', 'letterSpacing': '1px'}),
        ...
        # --- NOUVELLE BOX : MARGE CIBLE ---
        html.Div([
            html.Label("MARGE CIBLE (%)", style={'color': T_COLORS['text_main'], 'fontSize': '12px', 'letterSpacing': '1px'}),
        ...
        html.Button("PRICER EN TEMPS RÉEL", id='compute-btn', n_clicks=0, style={'height': '45px', 'padding': '0 30px', 'backgroundColor': T_COLORS['accent_cyan'], 'color': '#000', 'border': 'none', 'borderRadius': '5px', 'fontWeight': 'bold', 'cursor': 'pointer', 'letterSpacing': '1px', 'flex': '0.5'})
    ]),
    
    # Message d'erreur potentiel
    html.Div(id='error-message', style={'color': T_COLORS['accent_red'], 'textAlign': 'center', 'marginBottom': '20px'}),

    # Conteneur des résultats qui sera rempli par le callback
    dcc.Loading(
        id="loading-1",
        type="cube",
...
# =============================================================================
# 6. CALLBACK : MOTEUR QUANTITATIF CONNECTÉ À YFINANCE
# =============================================================================
@app.callback(
    ...
    [State('ticker-input', 'value'),
     ...
     State('margin-input', 'value')] # <-- Ajout du State pour la marge
)
def update_dashboard(n_clicks, ticker_str, target_date_str, margin_val):
    ...
    try:
        # --- GESTION DYNAMIQUE DE LA MARGE ET DU BUDGET ---
        try:
            marge_cible = float(margin_val) / 100.0 if margin_val is not None else 0.02
        except ValueError:
            marge_cible = 0.02 # Fallback à 2% en cas d'erreur de saisie
            
        budget_max = N * (1 - marge_cible)

        # 1. RÉCUPÉRATION DES DONNÉES TEMPS RÉEL
        ticker = yf.Ticker(ticker_str.upper())
        hist = ticker.history(period="1d")
        if hist.empty:
            return "", f"Ticker {ticker_str} introuvable."
        S0 = hist['Close'].iloc[-1]
        
        # Récupération des maturités disponibles sur le marché
        ...
        # Taux sans risque (Approximation avec le Treasury US 3 Mois ^IRX)
        try:
        ...
        if q is None: 
            q = 0.0
        elif q > 0.20: # Si yfinance pète un plomb et renvoie > 20%
            q = q / 100.0 if q > 1 else 0.03 # Fallback de sécurité

        # =============================================================================
        # --- NOUVELLE GESTION ROBUSTE DES PRIX (BID/ASK) EN CASCADE ---
        # =============================================================================
        ...
        # 1. On sécurise la Volatilité Implicite (Sigma) sur la monnaie
        ...
        # 2. On calcule le prix THÉORIQUE Black-Scholes pour TOUTES les options (Vecteur)
        calls['BS_Price'] = bs_call(S0, calls['strike'], T, r, q, sigma)
        puts['BS_Price'] = bs_put(S0, puts['strike'], T, r, q, sigma)

        # 3. Logique de remplacement (Fallback)
        ...
        # 4. On nettoie et on fusionne pour ton solveur
        ...
        if options_data.empty:
            return "", "La chaîne d'options récupérée est vide."

        # =============================================================================
        # 2. SOLVERS (DÉTERMINATION DES PARAMÈTRES COMMERCIAUX) 
        # =============================================================================
        # --- PPPN ---
        capital_protege = 900
        zcb_cost_pppn = capital_protege * np.exp(-r * T) 
        budget_options_pppn = budget_max - zcb_cost_pppn
        ratio_pppn = N / S0
        ...
        cout_total_pppn = zcb_cost_pppn + ratio_pppn * (put_ask_pppn + call_ask_pppn)
        ...
        budget_dispo_airbag = (quantite_put * put_bid_airbag) - zcb_cost_airbag + budget_max
        ...
        call_cost = call_ask_airbag * (N / S0)
        p_optimal = (quantite_put * put_bid_airbag - zcb_cost_airbag + budget_max) / call_cost if call_cost > 0 else 0
        cout_total_airbag = zcb_cost_airbag - (quantite_put * put_bid_airbag) + (p_optimal * (N / S0) * call_ask_airbag)
        ...
        fig_paths.add_trace(go.Scatter(y=np.mean(S, axis=1), mode='lines', name='Moyenne', line=dict(width=3, color=T_COLORS['accent_cyan'])))
        fig_paths = apply_theme(fig_paths, f"TRAJECTOIRES DU SOUS-JACENT ({ticker_str.upper()}) - MONTE CARLO")

        def create_pnl_chart(data_product, title, color):
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=ST_range, y=data_product, name='Produit', line=dict(color=color, width=3)))
            fig.add_trace(go.Scatter(x=ST_range, y=pnl_stock, name=f'Achat Action {ticker_str.upper()}', line=dict(color=T_COLORS['text_main'], width=2, dash='dot')))
            fig.add_vline(x=S0, line_dash="dash", line_color="gray")
            return apply_theme(fig, title)

        fig_pnl_pppn = create_pnl_chart(eval_portfolio_MC(port_pppn, ST_range) - N, "P&L THÉORIQUE", T_COLORS['accent_cyan'])
        fig_pnl_airbag = create_pnl_chart(eval_portfolio_MC(port_airbag, ST_range) - N, "P&L THÉORIQUE", T_COLORS['accent_gold'])
        ...
        def create_3d(Z_data, color_scale, title):
            ...
            fig.update_layout(
                ...
                scene=dict(
                    xaxis=dict(title='Prix Sous-Jacent ($)', gridcolor=T_COLORS['grid_color'], backgroundcolor='rgba(0,0,0,0)'),
                    yaxis=dict(title='Temps (Années)', gridcolor=T_COLORS['grid_color'], backgroundcolor='rgba(0,0,0,0)'),
                    zaxis=dict(title='Valorisation MtM ($)', gridcolor=T_COLORS['grid_color'], backgroundcolor='rgba(0,0,0,0)')
        ...
        # =============================================================================
        table_data_pppn = [
            {'Instrument': 'Zéro-Coupon Bond', 'Action': 'ACHAT', 'Quantite': '-', 'Strike': '-', 'Prix_U': '-', 'Total': f"- {zcb_cost_pppn:.2f} $"},
            {'Instrument': 'Put Option', 'Action': 'ACHAT', 'Quantite': f"{ratio_pppn:.4f}", 'Strike': f"{best_k1:.2f} $", 'Prix_U': f"{put_ask_pppn:.2f} $", 'Total': f"- {ratio_pppn * put_ask_pppn:.2f} $"},
            {'Instrument': 'Call Option', 'Action': 'ACHAT', 'Quantite': f"{ratio_pppn:.4f}", 'Strike': f"{best_k2:.2f} $", 'Prix_U': f"{call_ask_pppn:.2f} $", 'Total': f"- {ratio_pppn * call_ask_pppn:.2f} $"},
            {'Instrument': 'BILAN FABRICATION', 'Action': '-', 'Quantite': '-', 'Strike': '-', 'Prix_U': 'Coût Total:', 'Total': f"- {cout_total_pppn:.2f} $"},
            {'Instrument': 'MARGE BANCAIRE', 'Action': 'PROFIT', 'Quantite': '-', 'Strike': '-', 'Prix_U': f"Vendu: {N:.2f} $", 'Total': f"+ {N - cout_total_pppn:.2f} $"}
        ]

        table_data_airbag = [
            {'Instrument': 'Zéro-Coupon Bond', 'Action': 'ACHAT', 'Quantite': '-', 'Strike': '-', 'Prix_U': '-', 'Total': f"- {zcb_cost_airbag:.2f} $"},
            {'Instrument': 'Put Option (Financement)', 'Action': 'VENTE', 'Quantite': f"{quantite_put:.4f}", 'Strike': f"{closest_put_strike:.2f} $", 'Prix_U': f"{put_bid_airbag:.2f} $", 'Total': f"+ {quantite_put * put_bid_airbag:.2f} $"},
            {'Instrument': 'Call Option (Participation)', 'Action': 'ACHAT', 'Quantite': f"{p_optimal * N / S0:.4f}", 'Strike': f"{closest_call_strike:.2f} $", 'Prix_U': f"{call_ask_airbag:.2f} $", 'Total': f"- {p_optimal * (N/S0) * call_ask_airbag:.2f} $"},
            {'Instrument': 'BILAN FABRICATION', 'Action': '-', 'Quantite': '-', 'Strike': '-', 'Prix_U': 'Coût Total:', 'Total': f"- {cout_total_airbag:.2f} $"},
            {'Instrument': 'MARGE BANCAIRE', 'Action': 'PROFIT', 'Quantite': '-', 'Strike': '-', 'Prix_U': f"Vendu: {N:.2f} $", 'Total': f"+ {N - cout_total_airbag:.2f} $"}
        ]

        content = html.Div([
            
            # Affichage des données de marché Live
            html.Div(style={**GLASS_STYLE, 'marginBottom': '40px', 'padding': '20px', 'display': 'flex', 'justifyContent': 'space-around'}, children=[
                ...
                html.Div(f"Maturité: {expiration_date} (T={T:.2f} ans)", style={'color': T_COLORS['text_kpi']}),
                html.Div(f"IV (Sigma): {sigma*100:.1f} %", style={'color': T_COLORS['accent_gold']}),
                html.Div(f"Taux sans risque: {r*100:.2f} %", style={'color': T_COLORS['text_kpi']}),
                html.Div(f"Dividende: {q*100:.2f} %", style={'color': T_COLORS['text_kpi']}),
            ]),
            ...
            html.Div(style={**GLASS_STYLE, 'marginBottom': '50px', 'padding': '30px'}, children=[
                ...
                html.Div([
                    kpi_card("Prix de Vente", f"{N} $", T_COLORS['text_kpi']),
                    kpi_card("Garantie Capital", f"{capital_protege} $", T_COLORS['accent_cyan']),
                    kpi_card("Rendement Espéré", f"{kpi_p[0]:.2f} %", T_COLORS['accent_green']),
                    kpi_card("Risque Perte (<1000$)", f"{kpi_p[2]:.2f} %", T_COLORS['accent_red']),
                    kpi_card("Marge Nette", f"{N - cout_total_pppn:.2f} $", T_COLORS['accent_green'])
                ], style={'display': 'flex', 'backgroundColor': 'rgba(0,0,0,0.2)', 'borderRadius': '8px', 'marginBottom': '30px'}),
                
                html.H3("DÉTAIL DE STRUCTURATION (TERM SHEET)", style={'color': T_COLORS['text_main'], 'fontSize': '14px', 'letterSpacing': '1px'}),
            ...
            html.Div(style={**GLASS_STYLE, 'marginBottom': '50px', 'padding': '30px'}, children=[
                ...
                html.Div([
                    kpi_card("Prix de Vente", f"{N} $", T_COLORS['text_kpi']),
                    kpi_card("Niveau Airbag (A)", f"{A:.2f} $", T_COLORS['accent_gold']),
                    kpi_card("Participation (p)", f"{p_optimal*100:.2f} %", T_COLORS['accent_gold']),
                    kpi_card("Risque Perte (<1000$)", f"{kpi_a[2]:.2f} %", T_COLORS['accent_red']),
                    kpi_card("Marge Nette", f"{N - cout_total_airbag:.2f} $", T_COLORS['accent_green'])
                ], style={'display': 'flex', 'backgroundColor': 'rgba(0,0,0,0.2)', 'borderRadius': '8px', 'marginBottom': '30px'}),
